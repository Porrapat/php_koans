...
class AboutAssociativeArray extends TestCase
{
    ...
    {
        $empty_arr = [];
        $this->assertEquals('array', gettype($empty_arr));
        $this->assertEquals([], $empty_arr);
        $this->assertEquals(0, count($empty_arr));
    }
    ...
    public function test_accessing_associative_arrays()
    {
        ...
        $this->assertEquals('dos', $arr['two']);
        $this->assertEquals(null, $arr['doesnt_exist'] ?? null);
    }
    ...
    {
        $arr = [ 'one' => 'uno', 'two' => 'dos' ];
        $this->assertEquals(2, count(array_keys($arr)));
        $this->assertTrue(in_array('one', array_keys($arr)));
        $this->assertTrue(in_array('two', array_keys($arr)));
        $this->assertEquals('array', gettype(array_keys($arr)));
    }
    ...
    {
        $arr = [ 'one' => 'uno', 'two' => 'dos' ];
        $this->assertEquals(2, count(array_values($arr)));
        $this->assertTrue(in_array('uno', array_values($arr)));
        $this->assertTrue(in_array('dos', array_values($arr)));
        $this->assertEquals('array', gettype(array_values($arr)));
    }
    ...
    public function test_combining_associative_arrays()
    {
        ...
        $this->assertNotEquals($arr, $new_arr);

        $expected = [ "jim" => 54, "amy" => 20, "dan" => 23, "jenny" => 26 ];
        $this->assertEquals($expected, $new_arr);
    }
}
